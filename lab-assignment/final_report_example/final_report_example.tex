...
\textbf{How the system connects to BuildSim.} Device-facing processes and the
dashboard use BuildSim's API; the autonomous service commands the actuator
process rather than impersonating a device. Nothing else touches the building
state directly:
...
\begin{center}\includegraphics[height=0.9\textheight]{figures/container.png}\end{center}
...
QoS 1, every 5 s \\
Service → actuator & command + reason & REST &
\texttt{POST\ /commands} & actuator validates authority, range, and freshness \\
Actuator → BuildSim & apply state & REST &
\texttt{PUT\ /api/actuators/\{id\}/state} & 200 ok; 404 if not registered \\
Simulator ← BuildSim & read state & REST &
\texttt{GET\ /api/actuators/\{id\}} & closes the physical loop \\

...
func setupTestRouter() (*gin.Engine, *store.MemoryStore) {
	...
	api := r.Group("/api")
	{
		...
		vh := &handlers.VisualizationHandlers{Store: st, Hub: hub}
		api.GET("/room-layers", vh.GetRoomLayers)
		api.PUT("/room-layers", vh.SetRoomLayers)
		api.GET("/effects", vh.GetEffects)
		api.PUT("/effects", vh.SetEffects)
		api.GET("/entities", vh.GetEntities)
		api.PUT("/entities", vh.SetEntities)
		api.GET("/room-appearance", vh.GetRoomAppearance)
		api.PUT("/room-appearance", vh.SetRoomAppearance)
		api.GET("/alerts", vh.GetAlerts)
		api.PUT("/alerts", vh.SetAlerts)
		api.GET("/doors", vh.GetDoors)
		api.PUT("/doors", vh.SetDoors)
	...
	wsHandler := &handlers.SessionHandlers{Store: st, Hub: hub}
	r.GET("/ws/:id", wsHandler.HandleWebSocket)
...
func TestSessionLifecycle(t *testing.T) {
	...
	// Set occupancy
	occ := map[string]model.RoomOccupancy{
		"level0/R002": {Persons: []model.Person{{ID: "p1", Name: "Alice"}}, Aliens: []model.Alien{}},
	}
...
func TestSessionMutationResponsesExposeMonotonicVersions(t *testing.T) {
	...
	updates := []struct {
		...
		{"viewport", model.Viewport{Mode: "2d", Floor: "level0", Zoom: 1}},
		{"highlights", []model.RoomHighlight{{Level: "level0", Room: "R001", Color: "#fff", Opacity: 1}}},
		{"occupancy", map[string]model.RoomOccupancy{"level0/R001": {}}},
		{"route", model.RouteResult{Path: []model.RouteNode{{RoomID: 0, Name: "R001"}}}},
		{"coverage", []model.CoverageZone{{ID: "zone", Level: "level0", Room: "R001", Radius: 2, Opacity: 0.2}}},
	}
	for index, update := range updates {
		response := doRequest(router, http.MethodPut, "/api/sessions/"+session.ID+"/"+update.path, update.body)
		if response.Code != http.StatusOK {
			t.Fatalf("%s: %d %s", update.path, response.Code, response.Body.String())
		}
		var body struct {
		...
		if body.Version != want {
			t.Fatalf("%s version=%d, want %d", update.path, body.Version, want)
		}
	}
	stored, ok := memory.GetSession(session.ID)
	if !ok || stored.Version != int64(len(updates)) {
...
func TestMultiFloorRouteRequiresQualifiersForDuplicateNames(t *testing.T) {
	router, memory := setupTestRouter()
	memory.SetMultiFloorGraph(&model.NavGraph{
		Nodes: []model.NavNode{
			{ID: 1, Name: "Shared", Type: "room", Level: "level0"},
			{ID: 2, Name: "Target", Type: "room", Level: "level0"},
			{ID: 3, Name: "Shared", Type: "room", Level: "level1"},
			{ID: 4, Name: "Target", Type: "room", Level: "level1"},
		},
		Edges: []model.NavEdge{{From: 1, To: 2, Weight: 1}, {From: 3, To: 4, Weight: 2}},
	})

	response := doRequest(router, http.MethodGet, "/api/graph/route?type=walkable&from_name=Shared&to_name=Target", nil)
	if response.Code != http.StatusBadRequest || !bytes.Contains(response.Body.Bytes(), []byte("provide from_level")) {
		t.Fatalf("ambiguous route=%d %s", response.Code, response.Body.String())
	}

	response = doRequest(router, http.MethodGet, "/api/graph/route?type=walkable&from_name=Shared&from_level=level1&to_name=Target&to_level=level1", nil)
	if response.Code != http.StatusOK {
		t.Fatalf("qualified route=%d %s", response.Code, response.Body.String())
	}
	var route model.RouteResult
	parseJSON(response, &route)
	if route.Distance != 2 || route.Path[0].Level != "level1" {
		t.Fatalf("qualified route=%+v", route)
	}
}

func TestEquipmentMutationVersionMatchesStoredVersionAndHeader(t *testing.T) {
	router, memory := setupTestRouter()
	response := doRequest(router, http.MethodPost, "/api/equipment", model.Equipment{
		ID: "versioned", Level: "level0", Room: "R001",
		Sensors: []model.Sensor{{ID: "versioned-sensor", DataType: "text"}},
	})
	if response.Code != http.StatusCreated {
		t.Fatal(response.Body.String())
	}
	var equipment model.Equipment
	parseJSON(response, &equipment)
	if equipment.Version != 1 || response.Header().Get("X-BuildSim-Version") != "1" {
		t.Fatalf("create version=%d header=%q", equipment.Version, response.Header().Get("X-BuildSim-Version"))
	}

	response = doRequest(router, http.MethodPut, "/api/sensors/versioned-sensor/value", model.SensorValue{DataType: "text", Value: "final"})
	if response.Code != http.StatusOK || response.Header().Get("X-BuildSim-Version") != "2" {
		t.Fatalf("update=%d header=%q body=%s", response.Code, response.Header().Get("X-BuildSim-Version"), response.Body.String())
	}
	stored, _ := memory.GetEquipment("versioned")
	if stored.Version != 2 || memory.GetEquipmentVersion() != 2 {
		t.Fatalf("stored=%d global=%d", stored.Version, memory.GetEquipmentVersion())
	}
}

func TestConcurrentSensorMutationsReturnTheirExactVersions(t *testing.T) {
	router, memory := setupTestRouter()
	created := doRequest(router, http.MethodPost, "/api/equipment", model.Equipment{
		ID: "concurrent-version", Level: "level0", Room: "R001",
		Sensors: []model.Sensor{{ID: "concurrent-version-sensor", DataType: "text"}},
	})
	...
	const updates = 100
	versions := make(chan int64, updates)
	errorsFound := make(chan string, updates)
	...
	for update := 0; update < updates; update++ {
		...
		go func() {
			defer wait.Done()
			response := doRequest(router, http.MethodPut, "/api/sensors/concurrent-version-sensor/value",
				model.SensorValue{DataType: "text", Value: strconv.Itoa(update)})
			if response.Code != http.StatusOK {
				errorsFound <- response.Body.String()
				return
			}
			version, err := strconv.ParseInt(response.Header().Get("X-BuildSim-Version"), 10, 64)
			if err != nil {
	...
	for message := range errorsFound {
		t.Fatalf("concurrent mutation: %s", message)
	}
	seen := make(map[int64]bool, updates)
	for version := range versions {
		if version < 2 || version > updates+1 || seen[version] {
			t.Fatalf("invalid or duplicate version %d", version)
		}
		seen[version] = true
	}
	if len(seen) != updates {
		t.Fatalf("received %d unique versions, want %d", len(seen), updates)
	}
	stored, _ := memory.GetEquipment("concurrent-version")
	if stored.Version != updates+1 || memory.GetEquipmentVersion() != updates+1 {
		t.Fatalf("stored=%d global=%d, want %d", stored.Version, memory.GetEquipmentVersion(), updates+1)
	}
}
...
func TestGlobalOccupancy(t *testing.T) {
	...
	// Set occupancy
	occ := map[string]model.RoomOccupancy{
		"level0/R002": {Persons: []model.Person{{ID: "p1", Name: "Alice"}}, Aliens: []model.Alien{{ID: "x1"}}},
	}
	...
	// Verify persists
	w = doRequest(r, "GET", "/api/occupancy", nil)
	var got map[string]model.RoomOccupancy
	parseJSON(w, &got)
	if len(got) != 1 {
		t.Fatalf("expected 1 room, got %d", len(got))
	}
	if len(got["level0/R002"].Persons) != 1 || got["level0/R002"].Persons[0].Name != "Alice" {
		t.Fatalf("expected Alice, got %v", got["level0/R002"])
	}
	if len(got["level0/R002"].Aliens) != 1 {
		t.Fatalf("expected 1 alien, got %d", len(got["level0/R002"].Aliens))
	}
...
func TestConcurrentSessionUpdates(t *testing.T) {
	...
	go func() {
		for i := 0; i < 50; i++ {
			occ := map[string]model.RoomOccupancy{
				"level0/R001": {Persons: []model.Person{{ID: "p1", Name: "A"}}, Aliens: []model.Alien{}},
			}

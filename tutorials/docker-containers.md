...
For Compose to build the sensor, its source from Example 2 lives in its own folder. The
example also assumes the course repository's `buildingsim/` directory is next to the student
project; adjust that relative path if the folders are elsewhere:

```
workspace/
├── buildingsim/         # supplied course repository
└── project/
    ├── compose.yaml
...
The file below starts BuildSim and builds the sensor image once, then runs **two instances**
of it. Only the browser-facing BuildSim port is published to the laptop. Containers reach
BuildSim by its Compose service name:
...
services:
  ...
  temp-a109:
    ...
    ports: ["9000:9000"]
    depends_on:
      buildsim: { condition: service_healthy }
...
curl -s localhost:9090/healthz   # BuildSim is ready
...
Compose places all containers on one network where each is reachable by its **service
name**. A consumer reads `http://temp-a109:9000/value`, while every student service uses
`http://buildsim:9090` for the supplied API. Do not configure a container with
`http://127.0.0.1:9090`: inside that container, loopback points back to itself. The browser
uses `http://127.0.0.1:9090` because the port mapping exposes BuildSim only on the laptop's
loopback interface.

The example sensor does not yet use `BUILDSIM_URL`; add that configuration when it publishes
its reading. A real broker and data store join the same file later. Keeping all services on
this shared foundation makes startup order, health, names, and network boundaries explicit.

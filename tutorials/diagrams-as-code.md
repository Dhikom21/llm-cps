...
control: Ventilation Control {
  ...
  decider -> actuator: validated command
}

buildsim: BuildSim

buildsim -> control.sensors: read sensor state
control.actuator -> buildsim: apply actuator state
```

<figure class="diagram">
<img src="figures/diagrams-as-code-fig02.svg" alt="A D2 container diagram: a dashed Ventilation Control boundary contains sensor, history, autonomous decision, and actuator services; only the device-facing services access BuildSim">
...
The `style.fill: transparent` keeps the boundary box from painting a solid colour over the diagram, so the inner containers stand out, a small detail that makes C4 diagrams much more readable. Notice the architectural boundary: the decider proposes a command to an independently deployable actuator service. The actuator service owns validation, device communication, retries, and reporting applied state to BuildSim. A direct decider-to-BuildSim arrow would hide those responsibilities and would contradict the project process boundary.
